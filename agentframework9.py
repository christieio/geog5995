...
class Agent(): 
    ...
    def move (self):
        ...
        #otherwise
            ...
            #minus one from the y value
            self.y = (self.y - 1) % 100
        #if random number is less than one then:
        if random.random() < 0.5:
            #add one to the x value
            self.x = (self.x + 1) % 100
        #otherwise
        else:
            #minus one from the x value
            self.x = (self.x - 1) % 100
    ...
    def eat(self): 
        #if the environment is greater than ten then:
        if self.environment[self.y][self.x] > 10:
            #minus ten from the value
            self.environment[self.y][self.x] -= 10
            #add ten to the agent store
            self.store += 10
