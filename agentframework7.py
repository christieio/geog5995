...
class Agent(): 
    ...
    def distance_between (self, b):
        #return distance between (self x coordinate and b's x coordinate)**2
        #plus distance between (self y coordinate and b's y cooridinate)**2
        #all square rooted
    ...
    def share_with_neighbours (self, neighbourhood):
        ...
        if distance < neighbourhood:
            ...
            #total used to represent sum as sum has other usage in python
            total = self.store + self.agents[i].store
            #divide total by two to calculate average
            average = total / 2
            #self.store = average
            self.store = average
            #agent.store = average
